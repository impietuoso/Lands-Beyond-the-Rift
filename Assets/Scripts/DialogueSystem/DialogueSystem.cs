...
namespace DialogueSystem {
    public class DialogueSystem : MonoBehaviour {
        ...
        IEnumerator TypeDialogue(DialogueStructure dialogue) {
            ...
            if(leftPortrait) leftPortrait.overrideSprite = dialogue.leftCharacter? dialogue.leftCharacter.portrait : null;
            if(rightPortrait) rightPortrait.overrideSprite = dialogue.rightCharacter ? dialogue.rightCharacter.portrait : null;
            dialogueText.text = dialogue.dialogueText;
            dialogueText.maxVisibleCharacters = 0;
            yield return null;
            var parsedText = dialogueText.GetParsedText();
            yield return dialogue.effect?.Play(this);
